...
class LLMHandler:
    ...
    async def _handle_message(self, ctx: _Ctx) -> None:
        ...
        logger.info(
            "llm: provider={} model={} history_turns={} text={}",
            provider_name,
            model or "default",
            len(messages) // 2,
        ...
        try:
            result = await self.framework.invoke_provider(**invoke_kwargs)  # type: ignore[arg-type]
        ...
        if result.error:
            ...
            if err.retryable:
                ...
                try:
                    retry_result = await self.framework.invoke_provider(**invoke_kwargs)  # type: ignore[arg-type]
