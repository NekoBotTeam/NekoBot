...
async def initialize_webui():
    ...
    if not CONFIG.get("webui_enabled", True):
        logger.info("WebUI 已禁用，跳过前端文件初始化")
        return

    if not CONFIG.get("webui_api_enabled", True):
        logger.info("WebUI API 已禁用，但前端文件仍会初始化")

    try:
        from .webui_manager import initialize_webui as webui_init

        custom_proxy = CONFIG.get("github_proxy")
        version = CONFIG.get("webui_version")

        logger.info("正在初始化 WebUI...")
        success = await webui_init(custom_proxy=custom_proxy, version=version)

        if success:
            logger.info(f"WebUI 初始化完成 (版本: {get_webui_version()})")
        else:
            logger.warning("WebUI 初始化失败，但服务将继续运行")
    except Exception as e:
        logger.error(f"WebUI 初始化出错: {e}")
        logger.warning("服务将继续运行，但 WebUI 可能不可用")
